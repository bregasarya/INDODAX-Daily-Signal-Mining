...
if(isset($_GET['dari_tgl']) && isset($_GET['sampai_tgl'])){
    $dari_tgl = $_GET['dari_tgl'];
    $sampai_tgl = $_GET['sampai_tgl'];
    $jumlahDataPerHalaman = 3000;
    $result = mysqli_query($mysqli, "SELECT * FROM btc WHERE tanggal BETWEEN  '$dari_tgl' AND '$sampai_tgl'");
    header("Content-type: application/vnd-ms-excel");
    header("Content-Disposition: attachment; filename=Data_Filter_Tanggal.xls");
     
}else if(isset($_GET['level'])){
    $level = $_GET['level'];
    $jumlahDataPerHalaman = 3000;
    $result = mysqli_query($mysqli, "SELECT * FROM btc WHERE level='$level' ");
    header("Content-type: application/vnd-ms-excel");
    header("Content-Disposition: attachment; filename=Data_Btc_Filter_Level.xls");
 
   
}else if(isset($_GET['dari_sinyal']) && isset($_GET['sampai_sinyal'])){
    $dari_sinyal = $_GET['dari_sinyal'];
    $sampai_sinyal = $_GET['sampai_sinyal'];
    $jumlahDataPerHalaman = 3000;
    $result = mysqli_query($mysqli, "SELECT * FROM btc WHERE sinyal BETWEEN  '$dari_sinyal' AND '$sampai_sinyal'");
    header("Content-type: application/vnd-ms-excel");
    header("Content-Disposition: attachment; filename=Data_Btc_Filter_Sinyal.xls");
 
}else if(isset($_GET['fHarga'])){
    $dari_harga = $_GET['dari_harga'];
    $sampai_harga = $_GET['sampai_harga'];
    $jumlahDataPerHalaman = 3000;
    $result = mysqli_query($mysqli, "SELECT * FROM btc WHERE hargaidr BETWEEN  '$dari_harga' AND '$sampai_harga'");
    header("Content-type: application/vnd-ms-excel");
    header("Content-Disposition: attachment; filename=Data_Btc_Filter_Harga.xls");
